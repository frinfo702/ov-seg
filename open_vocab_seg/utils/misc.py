...
class NestedTensor:
    def __init__(self, tensors: Any, mask: Optional[Tensor]) -> None:
...
def _onnx_nested_tensor_from_tensor_list(tensor_list: list[Tensor]) -> NestedTensor:
    ...
    for i in range(tensor_list[0].dim()):
        max_size_i = torch.max(
            torch.stack([img.shape[i] for img in tensor_list]).to(torch.float32)  # pyright: ignore[reportArgumentType]
        ).to(torch.int64)
...
def get_gt_binary_masks(gt_semseg: Tensor) -> Tensor:
